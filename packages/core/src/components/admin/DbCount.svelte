<script>
    ...
    import { setParams } from "$core/lib.js";
    ...
    // 검색 관련!!
    let startDate = $state(
        moment.utc().subtract(4, "days").format("YYYY-MM-DD"),
    );
    let endDate = $state(moment.utc().format("YYYY-MM-DD"));
    let seachVal = $state("");

    $effect(() => {
        dateArr = siteData.dateArr;
        pages = siteData.pageList;
        siteCountInfoList = siteData.site_count_info_list;
        allPageCount = siteData.allPageCount;
    ...
    function searchSubmit(e) {
        ...
        const moveUrl = setParams(option, true);
        goto(moveUrl, { replaceState: true, invalidateAll: true });
    }
    ...
    function movePage() {
        ...
        const moveUrl = setParams({ page: setPage });
        goto(moveUrl, { replaceState: true, invalidateAll: true });
    }
</script>
