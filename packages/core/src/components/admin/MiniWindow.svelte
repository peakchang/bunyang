<script>
    ...
    // import OneImg from "$lib/components/OneImg.svelte";
    ...
    let modifyImgArr = $state(
        siteData.hyData.hy_image_list
            ? siteData.hyData.hy_image_list.split(",")
            : [],
    );

    onMount(() => {
        hyData = siteData.hyData;
    });
...
<div class="suit-font px-3">
    ...
    <table class="w-full">
        <tbody>
            ...
            <tr>
                <th
                    class="in-th text-sm bg-slate-100 border-slate-300 whitespace-nowrap"
                >
                    추가 스크립트
                </th>
                <td class="in-td">
            ...
            <tr>
                ...
                <td class="in-td">
                    <div>
                        <OneImageUpload
                            imgPath={hyData.hy_card_image}
                            sendImgPath={(e) => {
                                if (e.imgPath) {
                                    hyData.hy_card_image = e.imgPath;
                                } else {
                                    hyData.hy_card_image = null;
                                }
                            }}
                            domainFolder={`mini1/${hyData.hy_num}`}
                        ></OneImageUpload>
                    </div>
                </td>
            </tr>
            <tr>
                ...
                <td class="in-td">
                    <div>
                        <OneImageUpload
                            imgPath={hyData.hy_main_image}
                            sendImgPath={(e) => {
                                if (e.imgPath) {
                                    hyData.hy_main_image = e.imgPath;
                                } else {
                                    hyData.hy_main_image = null;
                                }
                            }}
                            domainFolder={`mini1/${hyData.hy_num}`}
                        ></OneImageUpload>
    ...
    <div class="mt-3 p-3 border rounded-md">
        ...
        <SortableImg
            folder={`mini1/${hyData.hy_num}`}
            updateImg={(e) => {
                const tempImgArr = e.map((val) => val.url);
                hyData["hy_image_list"] = tempImgArr.join(",");
            }}
            loadImgArr={modifyImgArr}
        ></SortableImg>
    </div>
