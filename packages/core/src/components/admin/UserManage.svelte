<script>
    ...
    import { isHashedPassword, setParams } from "$core/lib.js";
    ...
    $effect(() => {
        users = siteData.user_datas;
        managers = siteData.manager_datas;
    ...
    function userManageSearch(e) {
        ...
        if (userRate != 0 || searchName || searchEmail) {
            ...
            const getMoveUrl = setParams(option, true);
            goto(getMoveUrl, { replaceState: true, invalidateAll: true });
        } else if (userRate == 0 && !searchName && !searchEmail) {
            const getMoveUrl = setParams({}, true);
            goto(getMoveUrl, { replaceState: true, invalidateAll: true });
        }
        nowPage = 1;
    }
    ...
    function movePage() {
        ...
        const getMoveUrl = setParams({ page: setPage });
        goto(getMoveUrl, { replaceState: true, invalidateAll: true });
    }
</script>
