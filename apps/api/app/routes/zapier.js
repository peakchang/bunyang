...
import { mailSender, aligoKakaoNotification_formanager_top } from "../back-lib/lib.js";
...
zapierRouter.post('/topby_tiktok', async (req, res) => {
    console.log('진입함?!?!?!?!?!??!?!');
    const body = req.body;
    let status = true;
...
zapierRouter.post('/richby', async (req, res) => {
    let status = true;
    const body = req.body;
    try {
        const get_temp_phone = body['raw__phone_number'];
        ...
        for (const key in body) {
            if (key.includes('raw__etc1') && !etcInsertStr.includes('af_mb_etc1')) {
                etcInsertStr = etcInsertStr + `, af_mb_etc1`;
                etcValuesStr = etcValuesStr + `, '${body[key]}'`;
                addEtcMessage = addEtcMessage + `// 기타 정보1 : ${body[key]}`
            } else if (key.includes('raw__etc2') && !etcInsertStr.includes('af_mb_etc2')) {
                etcInsertStr = etcInsertStr + `, af_mb_etc2`;
                etcValuesStr = etcValuesStr + `, '${body[key]}'`;
                addEtcMessage = addEtcMessage + `// 기타 정보2 : ${body[key]}`
            } else if (key.includes('raw__etc3') && !etcInsertStr.includes('af_mb_etc3')) {
...
zapierRouter.post('/withby', async (req, res) => {
    ...
    try {
        ...
        if (etcValuesStr) {
            ...
            formInertSql = `INSERT INTO application_form (af_form_name, af_form_type_in, af_form_location, af_mb_name, af_mb_phone , af_created_at) VALUES ('${reFormName}','분양','FB','${dbName}','${get_phone}','${nowStr}');`;
        }
